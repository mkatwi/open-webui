...
def sanitize_stt_supported_content_types(content_types: Optional[list[str]]) -> list[str]:
    return [
        content_type.strip()
...
def get_stt_supported_content_types(content_types: Optional[list[str]]) -> list[str]:
    return (
        sanitize_stt_supported_content_types(content_types)
        or DEFAULT_STT_SUPPORTED_CONTENT_TYPES.copy()
    )
...
async def update_audio_config(
    ...
    request.app.state.config.STT_SUPPORTED_CONTENT_TYPES = (
        sanitize_stt_supported_content_types(form_data.stt.SUPPORTED_CONTENT_TYPES)
    )
...
def transcription(
    ...
    if not any(
        fnmatch(file.content_type or "", content_type)
