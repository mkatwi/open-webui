...
describe('getSafeIframeSrc', () => {
	it('allows absolute external http and https iframe sources', () => {
		expect(getSafeIframeSrc('https://example.com/embed?id=1&amp;view=full', 'https://app.test')).toBe(
			'https://example.com/embed?id=1&view=full'
		);
	...
	it('rejects relative and same-origin iframe sources', () => {
		...
		expect(getSafeIframeSrc('https://app.test/api/v1/auths/signout', 'https://app.test')).toBeNull();
	});
